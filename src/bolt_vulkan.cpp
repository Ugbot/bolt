...
bool Context::create_elementwise_pipeline(MatmulPipeline* out) noexcept {
    *out = MatmulPipeline{};
    if (device_ == nullptr) return false;
    // BLLM-179: 3 bindings (a, b, out) + 8-byte push constant {int n; int op;}.
    return create_pipeline_common(static_cast<VkDevice>(device_), queue_family_index_,
                                   shaders::kelementwise_spv, shaders::kelementwise_spv_size,
                                   /*binding_count=*/3, /*push_constant_size=*/8, WeightDType::F32,
                                   out);
}

bool Context::elementwise(const MatmulPipeline& pipeline, const BufferHandle& a,
                          const BufferHandle& b, const BufferHandle& out, int32_t n,
                          int32_t op) noexcept {
    if (device_ == nullptr || !pipeline.is_valid()) return false;
    if (a.buffer == nullptr || b.buffer == nullptr || out.buffer == nullptr || n <= 0) return false;
    VkDevice device = static_cast<VkDevice>(device_);

    VkDescriptorBufferInfo buffer_infos[3] = {
        {static_cast<VkBuffer>(a.buffer), 0, VK_WHOLE_SIZE},
        {static_cast<VkBuffer>(b.buffer), 0, VK_WHOLE_SIZE},
        {static_cast<VkBuffer>(out.buffer), 0, VK_WHOLE_SIZE},
    };
    VkWriteDescriptorSet writes[3]{};
    for (uint32_t i = 0; i < 3; ++i) {
