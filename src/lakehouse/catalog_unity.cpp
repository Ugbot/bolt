// bolt/src/lakehouse/catalog_unity.cpp — Databricks Unity Catalog REST.
//
// For W6: namespace = catalog (we conflate the catalog.schema 2-level hierarchy
// to a single level; list_tables takes namespace = "catalog.schema").
// table_path / commit_metadata → kCatNotImplemented (TODO W7+).
...
// Parse {"catalogs":[{"name":"..."}]} OR {"tables":[{"name":"..."}]} OR
// {"schemas":[{"name":"..."}]} — extract "name" of each element under key
// `array_key`.
bool parse_objects_array(const uint8_t* body, uint32_t len, bolt::Arena* arena,
                         const char* array_key, uint32_t array_key_len,
    ...
    *out_n = 0;
    ...
    while (i < idx.token_count &&
        ...
        bool match = tok_eq(idx, i, array_key, array_key_len);
        ++i;
                ...
                while (i < idx.token_count &&
                    ...
                    if (is_name && idx.tokens[i].type == bj::TokenType::String &&
                        *out_n < cap) {
                        ...
                        std::memcpy(out[*out_n].name, idx.src + idx.tokens[i].start, L);
                        out[*out_n].name[L] = '\0';
                        ++(*out_n);
                    }
                    i = skip_value(idx, i);
...
int vt_list_namespaces(void* impl_v, CatalogName* out, uint32_t cap,
    ...
    std::snprintf(req.url, sizeof(req.url),
                  "%s/api/2.1/unity-catalog/catalogs", impl->cfg.base_url);
    ...
    if (!parse_objects_array(resp.body, resp.body_len, impl->arena,
                             "catalogs", 8, out, cap, out_n)) return kCatIoError;
    return kCatOk;
}
...
int vt_list_tables(void* impl_v, const char* ns, CatalogName* out,
    ...
    if (dot == nullptr) {
        // Treat as catalog with schema=default.
        uint32_t n = static_cast<uint32_t>(std::strlen(ns));
        if (n >= sizeof(cat)) n = sizeof(cat) - 1u;
        std::memcpy(cat, ns, n); cat[n] = '\0';
        std::strncpy(sch, "default", sizeof(sch) - 1u);
    } else {
        uint32_t n = static_cast<uint32_t>(dot - ns);
        if (n >= sizeof(cat)) n = sizeof(cat) - 1u;
        std::memcpy(cat, ns, n); cat[n] = '\0';
        std::strncpy(sch, dot + 1, sizeof(sch) - 1u);
        sch[sizeof(sch) - 1u] = '\0';
    }
    bolt::net::HttpRequest req;
    std::memset(&req, 0, sizeof(req));
    std::strncpy(req.method, "GET", sizeof(req.method) - 1u);
    std::snprintf(req.url, sizeof(req.url),
                  "%s/api/2.1/unity-catalog/tables?catalog_name=%s&schema_name=%s",
                  impl->cfg.base_url, cat, sch);
    bolt::net::http_request_add_header(&req, "Accept", "application/json");
    bolt::net::HttpResponse resp;
    int rc = do_req(impl, &req, &resp);
    if (rc != kCatOk) return rc;
    if (!parse_objects_array(resp.body, resp.body_len, impl->arena,
                             "tables", 6, out, cap, out_n)) return kCatIoError;
    return kCatOk;
}
...
int vt_table_path(void* impl_v, const char* ns, const char* name, char* out,
    ...
    // TODO(W7+): GET /api/2.1/unity-catalog/tables/{full_name} → storage_location.
...
int vt_drop_table(void* impl_v, const char* ns, const char* name) noexcept {
    ...
    std::snprintf(req.url, sizeof(req.url),
                  "%s/api/2.1/unity-catalog/tables/%s.%s.%s",
                  impl->cfg.base_url, cat, sch, name);
    bolt::net::HttpResponse resp;
    return do_req(impl, &req, &resp);
}
