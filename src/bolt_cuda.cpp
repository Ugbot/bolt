// bolt_cuda.cpp -- BLLM-182: real CUDA kernels behind bolt::cuda (see
// bolt_cuda.h for the full contract). A near-1:1 mirror of bolt_rocm.cpp
// (CUDA and HIP are source-compatible); written against NVIDIA's documented
// CUDA runtime API (cudaGetDeviceCount/cudaSetDevice/cudaGetDeviceProperties/
// cudaMalloc/cudaFree/cudaMemcpy, `__global__`/`<<<>>>` kernel launch syntax
// -- all nvcc-standard). UNVALIDATED against a real nvcc/device: this dev box
// has NO NVIDIA GPU and NO CUDA toolkit (see bolt_cuda.h's status note and
// BLLM-182 tracker notes). This file is only compiled when CMakeLists.txt
// finds a real nvcc (BOLT_BUILD_CUDA); until then it is not part of any build
// and cannot break one. The ROCm twin (bolt_rocm.cpp) IS verified on real
// gfx1151 hardware, so this transcription's kernel logic is proven; only the
// s/hip/cuda/ API surface here awaits an NVIDIA device to validate.
...
// tier -- see bolt_cuda.h's matmul_dequant_int8_activation doc). Both
...
bool Context::create() noexcept {
    ...
    if (cudaSetDevice(chosen) != cudaSuccess) return false;

    cudaDeviceProp props{};
    if (cudaGetDeviceProperties(&props, chosen) != cudaSuccess) return false;
...
bool Context::allocate(uint64_t size_bytes, DeviceBuffer* out) noexcept {
    ...
    *out = DeviceBuffer{};
    ...
    void* ptr = nullptr;
    if (cudaMalloc(&ptr, size_bytes) != cudaSuccess) return false;
    out->ptr = ptr;
...
bool Context::copy_to_device(const void* host_src, DeviceBuffer* dst, uint64_t size_bytes) noexcept {
    ...
    return cudaMemcpy(dst->ptr, host_src, size_bytes, cudaMemcpyHostToDevice) == cudaSuccess;
}
...
bool Context::copy_to_host(const DeviceBuffer& src, void* host_dst, uint64_t size_bytes) noexcept {
    ...
    return cudaMemcpy(host_dst, src.ptr, size_bytes, cudaMemcpyDeviceToHost) == cudaSuccess;
}
